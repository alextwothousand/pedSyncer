...
import { Ped } from '../class/PedClass.js';
...
export function inDistanceBetweenPos(pos1, pos2, distance) {
    if (pos1.x - pos2.x > distance || pos1.y - pos2.y > distance || pos1.z - pos2.z > distance || pos1.x - pos2.x < -1 * distance || pos1.y - pos2.y < -1 * distance || pos1.z - pos2.z < -1 * distance) return false;
    if (typeof pos1 !== 'undefined' && typeof pos2 !== 'undefined' && typeof pos1.x !== 'undefined' && typeof pos2.x !== 'undefined') {
...
function _loadModel() {
    _loadModel = _asyncToGenerator(function*(classname) {
        let orig = classname;
        ...
        yield alt.loadModelAsync(classname);
    });
    return _loadModel.apply(this, arguments);
}
export function unloadModel(classname) {
    ...
    if (Ped.all.filter((ped)=>native.getEntityModel(parseInt(ped.scriptID)) == classname
    ).length == 0) native.setModelAsNoLongerNeeded(classname);
}
let pedToHash = {
    a_c_boar: 3462393972,
