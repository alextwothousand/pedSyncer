...
export function startPedControler() {
    ...
    alt.onServer("entitySync:updatePosition", (entityId, entityType, position)=>{
        ...
        if (typeof ped !== "undefined") {
            ...
            if (ped.scriptID != 0 && !inDistanceBetweenPos(ped.pos, position, 2)) {
                native.setEntityCoords(ped.scriptID, position.x, position.y, position.z, true, false, false, true); //todo
            }
        }
    });
            ...
            if (ped[key] != newEntityData[key]) {
                ...
                switch(key){
                    ...
                    case 'health':
                        ...
                        break;
                    case 'dead':
                        if (!native.isPedDeadOrDying(ped.scriptID, true)) native.setPedToRagdoll(ped.scriptID, -1, -1, 0, false, false, false);
                        break;
                    case 'freeze':
    ...
    function checkStreamedInVehicle() {
        try {
            ...
            for (let vehicle of alt.Vehicle.all){
                if (vehicle.scriptID != 0 && vehicle.hasSyncedMeta("ped")) {
    ...
    alt.setTimeout(()=>{
        alt.emitServer("pedSyncer:client:ready");
    }, 2000);
